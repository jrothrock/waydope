...
@Component({
  selector: 'apparel_post',
  templateUrl: 'apparel.post.component.html',
  providers: [ModalComponent,SystemMessagesComponent],
  entryComponents:[CommentsComponent]
})

export class ApparelPostComponent implements OnInit {
    ...
    post:any;
    apparel:any; // this is the actual apparel post. While the post above is used for the comments.
    id:any;
    postId:number;
    title:string;
    submitted_by:string;
    ...
    math:any=Math;
    window:any=window;
    specific:string; //uid of a comment that's currently being viewed
    ...
    constructor(private _auth: AuthService, private _voteService: VoteService, private _backend: BackendService, private _admin: AdminGuard, private _fb:FormBuilder, private _sysMessages: SystemMessagesComponent, private _cart: CartService, private _route: ActivatedRoute, private _http: Http, private _modal: ModalComponent, private _router: Router){
      this.routerSubscription = _router.events.subscribe(s => {
        if(s && s["state"] && this.loaded){
          ...
          if(url_bits.length === 5 && url_bits[4] != this.id){
            ...
            $("#apparel-post-container").removeClass('active-post');
            this.getApparelPost();
          }
        }
      })
      ...
      this.getApparelPost();
      this.voteCheck();
      if(window.outerWidth < 451) $("#share-button-apparel-post").removeClass("horizontal btn-large").addClass("btn-medium");
    };
    getApparelPost(){
      ...
      this.subscription = this._http.get(`${this._backend.SERVER_URL}/api/v1/apparel/${this.category}/${this.sub_category}/${this.id}`,{headers: headersInit}).subscribe(data => {
          ...
          // this.price = data.json().post.price;
          this.sale_price = data.json().post.sale_price;
          this.archived = data.json().post.archived;
          this.locked = data.json().post.locked;
          // this.quantity = parseInt(data.json().post.quantity);
          this.quantities = [];
          ...
          setTimeout(()=>{
              if(spinner) $("#loading-spinner-apparel-post").css({'display':'none'}); $("#apparel-post-container").addClass('active-post');
              if(!this.nsfw) $("#sfw-images-container").css({'display':'block'});
              else $("#nsfw-images-container").css({'display':'block'});
              $("#apparel-post-container").addClass('active-post');
              if(this.worked){
                let options = {disableZoom:'false',autoInside:768,zoomSizeWindow:'image'}
                if(this.photos.length && $('#main-photo-apparel').length) this.myInstance = new CloudZoom($('#main-photo-apparel'),options);
              }
              if(this.specific) {
                this.post = ['apparel', this.postId, this.category, this.specific] //this is passed to the comments component;
              ...
              this.checkMainPhoto();
              this.getImageWidth();
              _setMeta.setPost(data.json().post.title,`${data.json().post.description.substring(0,30)}...`,'apparel',data.json().post.main_category)
              //test
          },time);
    ...
    buyNow(){
      if(this.approved){
        if(this.apparel && this.quantitySelected ){
          // fourth variable (true) is to indicate that it's buy it now.
          // the third variable is for the edit
          this._cart.itemChange(this.apparel, {quantity:this.quantitySelected,size:this.sizeSelected,color:this.colorSelected},false,true);
        }
      } else {
  ...
  colorChange(e){
    ...
    toggleNSFW(type){
      ...
      let $apparel = $('#apparel-post-images-container');
      let height = $apparel.height()
      $apparel.css({'min-height':height})
      this.nsfw_ok = type;
      setTimeout(()=>{
    ...
    setVote(vote){
        ...
        this.voteSubscription = this._http.post(`${this._backend.SERVER_URL}/api/v1/votes`, body, {headers: headers}).subscribe(data => {
            ...
            this._voteService.change('apparel',this.postId,this.average_vote+change,data.json().user_vote);
    ...
    getImageWidth(){
      var container_width = ($(`.rest-photos-containers`).width()-30)/3
        for(let id = 0; id < this.photos.length;id++){
          ...
          // image_object.src = $(`#rest-photos-apparel-${id}`).attr("src");
          ...
          //     $(`#rest-photos-apparel-${id}`).height(148).width(new_width)
          // } else if (native_height > native_width && native_height <= 148) {
          //     $(`#rest-photos-apparel-${id}`).height(native_height).width(native_width)
          ...
          //     $(`#rest-photos-apparel-${id}`).height(new_height).width(container_width);
          // } else {
          //     $(`#rest-photos-apparel-${id}`).height(native_height).width(native_width)
          // }
          $(`#rest-photos-apparel-${id}`).height('100%').width('100%')
          $(`#rest-photos-apparel-${id}`).css({'display':'initial'});
        }
    }
    checkMainPhoto(){
        // let image_object = new Image();
        // image_object.src = $("#main-photo-apparel").attr("src");
        ...
        //     let new_width = native_width * multiplier;
        //     $("#main-photo-apparel").height(400).width(new_width)
        // } else if (native_height > native_width && native_height <= 400) {
        //     $("#main-photo-apparel").height(native_height).width(native_width)
        // } else if (native_width > native_height && native_width > 400){
        //     let multiplier = 400 / native_width;
        //     let new_height = native_height * multiplier;
        //     $("#main-photo-apparel").height(new_height).width(400);
        // } else {
        //     $("#main-photo-apparel").height(native_height).width(native_width)
        // }
        $("#main-photo-apparel").height('100%').width('100%')
        $("#main-photo-apparel").css({'display':'initial'});
        $("#main-photo-apparel").data('scaled',true);
    }
    ...
    removeFromCart(){
      if(this.apparel) this._cart.itemChange(this.apparel, {quantity:0,size:this.sizeSelected,color:this.colorSelected});
      this.addedToCart = false;
      let index = this.quantityLookUps.indexOf(`${this.postId}, ${this.sizeSelected}, ${this.colorSelected}`)
    ...
    addToCart(){
      if(this.approved){
        ...
        if(this.apparel && this.quantitySelected ) this._cart.itemChange(this.apparel, {quantity:this.quantitySelected,size:this.sizeSelected,color:this.colorSelected});
        this.addedToCart = true;
        let index = this.quantityLookUps.indexOf(`${this.postId}, ${this.sizeSelected}, ${this.colorSelected}`)
        if(index > -1){
      ...
      //  let headers = new Headers({
	    ...
	    // });
	    // let body = {"id":this.id}
	    // this.addToCartSubscription = this._http.post('${this._backend.SERVER_URL}/api/v1/cart/update', body, {headers: headers}).subscribe(data => {
      ...
      $(window).scroll(function(){
        if(!component.passedParams){
          if ($('.comments') && $(this).scrollTop() > ($('.comments').offset().top - 500)) {
            if(!component.passedParams){component.post = ['apparel', component.postId, component.category, component.sub_category, component.specific] } //this is passed to the comments component;
    ...
    editPost(){
		  this._router.navigateByUrl(`/apparel/edit?title=${this.title}&category=${this.category}&subcategory=${this.sub_category}&id=${this.id}`);
	  }
    lockInit(){
      this.datanotify=[this.postId,'apparel','check','lock'];
    }
    removeInit(){
      this.datanotify=[this.postId,'apparel','check','remove'];
    }
    adminInit(){
    ...
    changePhoto(index){
        ...
        $('#main-photo-apparel').attr('src', src);
        let options = {disableZoom:'false',autoInside:768,zoomSizeWindow:'image'}
        this.myInstance = new CloudZoom($('#main-photo-apparel'),options);
    }
