class Api::V1::Technology::CategoryController < ApplicationController
	...
	def index
		if request.headers["Category"]
			...
			#
            posts = $redis.get("technology_category_#{request.headers["Category"].parameterize}_#{request.headers["Subcategory"].parameterize}_#{offset}_#{time.strftime("%F")}_#{order.parameterize}_posts")
			if posts.nil?
                ...
                $redis.set("technology_category_#{request.headers["Category"].parameterize}_#{request.headers["Subcategory"].parameterize}_#{offset}_#{time.strftime("%F")}_#{order.parameterize}_posts",posts)
                $redis.rpush("technology_all_keys","technology_category_#{request.headers["Category"].parameterize}_#{request.headers["Subcategory"].parameterize}_#{offset}_#{time.strftime("%F")}_#{order.parameterize}_posts")
            end
    ...
    def read
		...
		if (!order || !time)
		 ...
		 if params[:category] === 'hot'
            ...
            sanitized_query = Product.escape_sql(["SELECT count(*) OVER (), #{App.getGoodColumns('technology',true,'p')} FROM products p WHERE post_type = 'technology' AND sold_out = false AND approved = true AND uploaded = true AND created_at >= ? AND featured = true AND flagged = false AND removed = false GROUP BY p.id ORDER BY #{order} OFFSET ? LIMIT 20", time.to_s,offset])
        elsif params[:subcategory] != '' && params[:subcategory] != nil
            sanitized_query = Product.escape_sql(["SELECT count(*) OVER (), #{App.getGoodColumns('technology',true,'p')} FROM products p WHERE post_type = 'technology' AND sold_out = false AND approved = true AND uploaded = true AND main_category = ? AND sub_category = ? AND created_at >= ? AND flagged = false AND removed = false GROUP BY p.id ORDER BY #{order} OFFSET ? LIMIT 20",params[:category], params[:subcategory], time.to_s,offset])
        else
            sanitized_query = Product.escape_sql(["SELECT count(*) OVER (), #{App.getGoodColumns('technology',true,'p')} FROM products p WHERE post_type = 'technology' AND sold_out = false AND approved = true AND uploaded = true AND main_category = ? AND created_at >= ? AND flagged = false AND removed = false GROUP BY p.id ORDER BY #{order} OFFSET ? LIMIT 20",params[:category], time.to_s,offset])
        end
    ...
    def update
        ...
        if params[:category] === 'hot'
            ...
            sanitized_query = Product.escape_sql(["SELECT count(*) OVER (), #{App.getGoodColumns('technology',true,'p')} FROM products p WHERE post_type = 'technology' AND sold_out = false AND approved = true AND uploaded = true AND created_at >= ? AND flagged = false AND removed = false AND featured = true GROUP BY p.id ORDER BY #{order} OFFSET ? LIMIT 20", time.to_s,offset])
        elsif params[:subcategory] != '' && params[:subcategory] != nil
            sanitized_query = Product.escape_sql(["SELECT count(*) OVER (), #{App.getGoodColumns('technology',true,'p')} FROM products p WHERE post_type = 'technology' AND sold_out = false AND approved = true AND uploaded = true AND main_category = ? AND flagged = false AND removed = false AND sub_category = ? AND created_at >= ? GROUP BY p.id ORDER BY #{order} OFFSET ? LIMIT 20",params[:category], params[:subcategory], time.to_s,offset])
        else
            sanitized_query = Product.escape_sql(["SELECT count(*) OVER (), #{App.getGoodColumns('technology',true,'p')} FROM products p WHERE post_type = 'technology' AND sold_out = false AND approved = true AND uploaded = true AND main_category = ? AND flagged = false AND removed = false AND created_at >= ? GROUP BY p.id ORDER BY #{order} OFFSET ? LIMIT 20",params[:category], time.to_s,offset])
        end
        posts = Product.find_by_sql(sanitized_query).as_json
        user = request.headers["Authorization"] ? User.find_by_token(request.headers["Authorization"].split(' ').last) : nil
        posts.each do |post|
